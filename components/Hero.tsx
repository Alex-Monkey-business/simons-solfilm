      ...
      >
        ...
        <div className="flex flex-col gap-10 lg:max-w-2xl lg:gap-8">
          ...
          {/* One row, four buttons, same height. The call button was its own
              full-width block above this row — which made it the loudest
              object on the page, and Simon is not certain a call is what
              every visitor wants first. It stays orange and stays first, so
              it is still the recommendation, but it no longer decides for
              the reader. The label above frames the row instead. */}
          <motion.div
            initial={{ opacity: 0, y: 12 }}
            animate={{ opacity: 1, y: 0 }}
              ...
              lead={
                <MagneticLink
                  ...
                  className={buttonClass({
                    variant: "primary",
                    size: "sm",
                    className: "group min-h-[44px] gap-2",
                  })}
                >
                  {/* A handset, not an arrow. An arrow promises navigation;
                      this starts a call. It was also the only glyph of the
                      four in this row that did not describe its own action. */}
                  <svg
                    viewBox="0 0 18 18"
                    aria-hidden
                    className="size-[15px] shrink-0 stroke-current transition-transform duration-300 group-hover:-rotate-12"
                    fill="none"
                    strokeWidth="1.4"
                    strokeLinecap="round"
